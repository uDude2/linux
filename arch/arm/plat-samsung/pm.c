...
struct device **s3c_cm_devices = NULL;
bool s3c_cm_resume_console = false;
...
static bool s3c_cm_suspend_again(void)
{
	...
	while (s3c_cm_devices && s3c_cm_devices[i]) {
		generic_subsys_pm_ops.resume(s3c_cm_devices[i]);
		i++;
	}
	...
	while (i > 0) {
		i--;
		generic_subsys_pm_ops.suspend(s3c_cm_devices[i]);
	}
